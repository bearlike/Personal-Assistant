...
Scope: this file applies to the `meeseeks-api/` package. It captures runtime behavior, hidden dependencies, and testing notes so changes stay safe and predictable.

## Runtime flow (what actually happens)
- Entry point: `meeseeks-api/backend.py` (HTTP API framework).
...
- Orchestration: calls `core.task_master.orchestrate_session(...)` with `auto_approve` and a shared `SessionStore`.
...
- Uses core logging (`core.common.get_logger`); log level controlled by core env.
